...
class EdMaverickApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Ed Maverick - Eduardo")
    ...
    def create_widgets(self):
        ...
        # Imagen del álbum
        album_image_path = "albumn3.png"  # Cambia la ruta si es necesario
        if os.path.exists(album_image_path):
            album_img = Image.open(album_image_path).resize((200, 200))
            album_img = ImageTk.PhotoImage(album_img)
            album_label = tk.Label(self.root, image=album_img, bg=self.bg_color)
            album_label.image = album_img
            album_label.place(x=180, y=20)

        # Descripción del álbum y artista
        album_description = (
            "Artista: Ed Maverick\n"
            "Álbum: Eduardo\n"
            "Lanzamiento: 26 de noviembre de 2021\n"
            "Género: Folk, Alternativo\n"
            "Duración: 44:10\n"
            "\n"
            "Eduardo es un álbum introspectivo que explora emociones crudas y "
            "sentimientos personales. Incluye temas como 'Nos Queda Mucho Dolor Por Recorrer', "
            "'Niño' y 'Fuentes de Ortiz'."
        )
        ...
        album3_image_path = "album3.jpg"  # Cambia la ruta si es necesario
        if os.path.exists(album3_image_path):
            album3_img = Image.open(album3_image_path).resize((200, 200))
            album3_img = ImageTk.PhotoImage(album3_img)
            album3_label = tk.Label(self.root, image=album3_img, bg=self.bg_color)
            album3_label.image = album3_img
            album3_label.place(x=180, y=45)            
    ...
    def load_songs(self):
        """Carga las canciones del álbum 'Eduardo'."""
        self.songs = [
            "¿por que lloras.mp3",
            "Contenta _ Atnetnoc.mp3",
            "días azules.mp3",
            "ensenada.mp3",
            "Gente.mp3",
            "gracias.mp3",
    ...
    def open_news(self):
        """Cierra esta ventana y abre la interfaz de Novedades."""
        self.root.destroy()
    ...
    def open_System(self):
        """Cierra esta ventana y abre la interfaz del Sistema."""
        self.root.destroy()
    ...
    def open_profile(self):
        """Cierra esta ventana y abre la configuración del perfil."""
        self.root.destroy()
        from conectar import PerfilApp
        new_window = tk.Tk()
        PerfilApp(new_window)
        new_window.mainloop()
